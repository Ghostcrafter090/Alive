...
class net:
    ...
    def sendEmail(userEmail, password, toEmail, inputType, messageData, server, port, ignoreSpam=False):
        try:
            
            if toEmail not in net.lastEmails.keys():
                net.lastEmails[toEmail] = [0, time.time()]
            
            if time.time() > (net.lastEmails[toEmail][1] + 15):
                net.lastEmails[toEmail][0] = 0
            
            if net.lastEmails[toEmail][0] > 4:
                net.lastEmails[toEmail][1] = time.time()
                print("Unnable to send email. Accidental spamming detected.")
                return 1
            
            net.lastEmails[toEmail][0] = net.lastEmails[toEmail][0] + 1
            net.lastEmails[toEmail][1] = time.time()
    ...
    def getJsonAPI(url, timeout=10, silent=False):
...
class clock:
    ...
    def unixToDateArray(unix):
        daten = datetime.fromtimestamp(float(unix), dater.timezone(dater.timedelta(hours=-3)))
        dateArray = [1970, 1, 1, 0, 0, 0]
        dateArray[0] = int(str(daten).split(" ")[0].split("-")[0])
        dateArray[1] = int(str(daten).split(" ")[0].split("-")[1])
        dateArray[2] = int(str(daten).split(" ")[0].split("-")[2])
