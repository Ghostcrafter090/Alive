...
execute as @e[tag=mining_pillager,type=pillager,scores={pillagerMiningDirection=5..,entityVerticalMotion=-10..,random100=..5},sort=random,limit=2] at @s if block ~ ~1 ~1 #gstools:air unless block ~ ~-1 ~ water unless block ~ ~ ~ water unless block ~ ~1 ~ water unless predicate gstools:sky run scoreboard players operation @s pillagerMiningDirection = @s random10
execute as @e[tag=mining_pillager,type=pillager,scores={pillagerMiningDirection=5..,entityVerticalMotion=-10..,random100=..5},sort=random,limit=2] at @s if block ~ ~1 ~1 #gstools:air unless block ~ ~-1 ~ water unless block ~ ~ ~ water unless block ~ ~1 ~ water unless predicate gstools:sky run scoreboard players operation @s pillagerMiningDirection /= @e[tag=gstools_worker,type=marker] 2
...
execute as @e[tag=mining_pillager,type=pillager,scores={random100=0..3},limit=1,sort=random] at @s if entity @e[tag=pillager_mining_outpost_component,type=marker,distance=0..30] unless block ~ ~-1 ~ water unless block ~ ~ ~ water unless block ~ ~1 ~ water if predicate gstools:sky run scoreboard players set @s pillagerMiningDirection 5
