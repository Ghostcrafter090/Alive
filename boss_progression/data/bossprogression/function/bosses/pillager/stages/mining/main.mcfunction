...
execute in overworld if entity @e[tag=gstools_worker,type=marker,scores={numberOfPillagers=..65}] as @e[type=marker,tag=gstools_vertical_cursor,distance=0..] at @s if block ~ ~ ~ #minecraft:air unless entity @e[tag=mining_pillager,type=pillager,distance=0..60] unless entity @e[type=player,type=pillager,distance=0..50] unless entity @e[type=villager,type=pillager,distance=0..100] unless entity @e[type=iron_golem,type=pillager,distance=0..100] unless entity @e[type=evoker,type=pillager,distance=0..100] unless entity @e[type=witch,type=pillager,distance=0..100] as @e[tag=gstools_cursor,sort=nearest,limit=1] if score @s verticalCursorWorker < @e[tag=gstools_worker,type=marker,limit=1] 60 run summon pillager ~ ~ ~ {PersistenceRequired:1b,CanPickUpLoot:1b,Tags:["mining_pillager"],equipment:{head:{id:"minecraft:iron_helmet",count:1},mainhand:{id:"minecraft:stone_pickaxe",count:1},offhand:{id:"minecraft:shield",count:1}}}
...
execute as @e[type=marker,tag=pillager_mining_outpost,tag=not_setup,sort=random] at @s if entity @e[tag=pillager_mining_outpost_component,distance=1..300,limit=1] run kill @s
execute as @e[type=marker,tag=pillager_mining_outpost,tag=not_setup,sort=random] at @s if entity @e[type=villager,distance=1..300,limit=1] run kill @s
execute as @e[type=marker,tag=pillager_mining_outpost,tag=not_setup,sort=random] at @s if entity @e[type=iron_golem,distance=1..300,limit=1] run kill @s
...
execute as @e[type=marker,tag=pillager_mining_outpost_component,tag=not_setup,sort=random,limit=1] at @s if entity @s[tag=do_setup,scores={random100=61..91}] run place template minecraft:pillager_outpost/feature_cage1
