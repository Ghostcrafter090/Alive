...
execute in overworld if entity @e[tag=gstools_worker,type=marker,scores={numberOfPillagers=..65}] as @e[type=marker,tag=gstools_vertical_cursor,distance=0..] at @s if block ~ ~ ~ #minecraft:air unless entity @e[tag=mining_pillager,type=pillager,distance=0..60] unless entity @e[type=player,distance=0..50] unless entity @e[type=villager,distance=0..100] unless entity @e[type=iron_golem,distance=0..100] unless entity @e[type=evoker,distance=0..100] unless entity @e[type=witch,distance=0..100] as @e[tag=gstools_cursor,sort=nearest,limit=1] if score @s verticalCursorWorker < @e[tag=gstools_worker,type=marker,limit=1] 60 run summon pillager ~ ~ ~ {PersistenceRequired:1b,CanPickUpLoot:1b,Tags:["mining_pillager"],equipment:{head:{id:"minecraft:iron_helmet",count:1},mainhand:{id:"minecraft:stone_pickaxe",count:1},offhand:{id:"minecraft:shield",count:1}}}
