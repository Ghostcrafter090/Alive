...
execute as @e[tag=attack_portal_node,tag=portal_generated,limit=1,sort=random,scores={attackPortalSpawnTic=900..}] if entity @s[scores={random100=80..85}] if entity @e[tag=gstools_worker,type=marker,limit=1,scores={numberOfOverworldNetherMobs=..65}] at @s run summon piglin_brute ~1 ~ ~1 {PersistenceRequired:1b}
