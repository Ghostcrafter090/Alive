...
execute as @e[tag=attack_portal_node,type=marker,tag=portal_generated,limit=1,sort=random,scores={attackPortalSpawnTic=300..}] if entity @e[tag=gstools_worker,type=marker,limit=1,scores={numberOfOverworldNetherMobs=..165}] at @s run tag @e[type=piglin,sort=nearest,tag=!nether_overworld,distance=0..] add nether_overworld
