...
execute as @e[type=marker,tag=attack_portal_node] at @s unless block ~ ~-1 ~ #gstools:air unless block ~1 ~-1 ~ #gstools:air unless block ~2 ~-1 ~ #gstools:air unless block ~3 ~-1 ~ #gstools:air if blocks ~2 ~ ~ ~1 ~3 ~ 2 0 0 all run tag @s add portal_gen
