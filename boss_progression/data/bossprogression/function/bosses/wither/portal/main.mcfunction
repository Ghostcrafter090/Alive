...
# Effects
execute if entity @e[tag=gstools_worker,type=marker,scores={enchantedWitherRoseHasBeenSpawned=1..1}] unless entity @e[tag=gstools_worker,type=marker,scores={witherHasBeenKilled=1..1}] as @e[tag=nether_overworld,tag=!immune_to_zombification] at @s if dimension minecraft:overworld run data merge entity @s {IsImmuneToZombification:1b,PersistenceRequired:1b} 
...
execute as @e[tag=!tile,sort=random,limit=1] if score @s random1000 < @s currentTemperature run schedule function bossprogression:bosses/wither/portal/util/fireout 1t append
