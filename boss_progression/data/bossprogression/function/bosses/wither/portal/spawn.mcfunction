...
execute as @e[type=marker,tag=attack_portal_node,tag=delete_portal,scores={attackPortalDeleteTic=..100}] at @s run particle reverse_portal ~1.5 ~2 ~ 0 0 0 1 100 force @a
...
execute if entity @e[tag=gstools_worker,type=marker,scores={enchantedWitherRoseHasBeenSpawned=1..1}] if entity @e[tag=gstools_worker,type=marker,scores={witherHasBeenKilled=1..1}] as @e[tag=attack_portal_node,tag=!pillager_made] run tag @s add delete_portal