...
execute unless entity @e[tag=gstools_worker,type=marker,scores={enhancedSurvivalLoaded=1..1}] as @a[scores={consumptionStrength=0..}] run scoreboard players add @s breathingSoundTic 1
...
execute unless entity @e[tag=gstools_worker,type=marker,scores={enhancedSurvivalLoaded=1..1}] as @a if score @s breathingSoundTic >= @s stamina run scoreboard players set @s breathingSoundTic 0