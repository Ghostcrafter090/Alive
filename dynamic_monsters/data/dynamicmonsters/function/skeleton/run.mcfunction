...
execute unless entity @s[scores={skeletonEquipmentVersionConflict=0..0}] run function dynamicmonsters:skeleton/version_conflict/equipment_1

execute unless entity @e[tag=gstools_worker,type=marker,scores={timeOfDay=0..12500,ticSecond=10..10}] if entity @s[type=skeleton] run effect give @s fire_resistance 10 1 true
...
execute if entity @s[scores={secondsTillExtinguished=1..},tag=!ignore_skeleton_flame] run tag @s remove equipment_removed
execute unless entity @s[scores={secondsTillExtinguished=1..}] if entity @s[tag=!ignore_skeleton_flame,tag=!equipment_removed] run data remove entity @s equipment.mainhand.components."minecraft:enchantments"."minecraft:flame"
execute unless entity @s[scores={secondsTillExtinguished=1..}] if entity @s[tag=!ignore_skeleton_flame,tag=!equipment_removed] run tag @s add equipment_removed
...
execute if entity @s[scores={random1000=..4}] as @s at @s run damage @s 0.0001 minecraft:mob_attack by @e[tag=!tile,sort=nearest,type=!skeleton,distance=1..,limit=1] from @e[tag=!tile,type=!skeleton,sort=nearest,distance=1..,limit=1]
