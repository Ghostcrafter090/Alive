...
execute if entity @e[tag=gstools_worker,type=marker,scores={random100=..2}] as @e[type=arrow,scores={secondsTillExtinguished=1..},sort=random,limit=1] at @s if block ~ ~ ~ #minecraft:air unless block ~ ~-1 ~ #minecraft:air run fill ~ ~ ~ ~ ~ ~ fire replace
