...
# Main
execute as @e[tag=gstools_cursor,type=marker] at @s if block ~ ~-2 ~ birch_leaves run summon marker ~ ~ ~ {Tags:['birch_tree_worker']}
...
execute as @e[tag=birch_tree_worker,type=marker] if entity @s[tag=birch_tree_locating,scores={treeWorkerAliveTime=300..}] as @e[tag=birch_tree_cursor,sort=nearest,limit=1] run tag @s add birch_tree_cursor_to_kill
...
execute as @e[type=marker,tag=birch_tree_cursor,scores={treeCursorAliveTime=500..}] run kill @s
execute as @e[type=marker,tag=birch_tree_worker,scores={treeWorkerAliveTime=500..}] run kill @s
