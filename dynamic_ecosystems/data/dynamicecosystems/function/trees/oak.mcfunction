...
# Main
execute as @e[tag=gstools_cursor,type=marker] at @s if block ~ ~-2 ~ oak_leaves run summon marker ~ ~ ~ {Tags:['oak_tree_worker']}
...
execute as @e[tag=oak_tree_worker,type=marker] if entity @s[tag=oak_tree_locating] run scoreboard players add @s treeWorkerAliveTime 1
