...
execute as @a at @s anchored eyes facing entity @e[type=text_display,tag=death_ghost,tag=death_ghost_setup,limit=1,sort=nearest,distance=0..64] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.07] if score @s EyesPosY >= @s EyesPosX run scoreboard players set @e[type=text_display,tag=death_ghost,tag=death_ghost_setup,limit=1,sort=nearest,distance=0..64] deathGhostBehaviourMode 3