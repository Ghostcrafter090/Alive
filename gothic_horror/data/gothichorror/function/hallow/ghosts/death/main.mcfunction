...
execute as @r if score @s horrorIndex > @s random1000 at @s unless entity @e[type=text_display,tag=death_ghost,distance=0..30,limit=1] run summon text_display ~ ~5 ~ {view_range:100f,glow_color_override:1b,width:4f,height:4f,billboard:"center",Tags:["death_ghost"],brightness:{sky:15,block:15},text:'{"text":"a","font":"gothichorror:images"}',background:16711680}
...
execute as @a at @s anchored eyes facing entity @e[type=text_display,tag=death_ghost,tag=death_ghost_setup,limit=1,sort=nearest,distance=0..64] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.07] if score @s EyesPosY >= @s EyesPosX run kill @e[type=text_display,tag=death_ghost,tag=death_ghost_setup,limit=1,sort=nearest,distance=0..64]