...
execute as @e[type=snowball,tag=gothic_ghost_snowball_thrown,tag=!sound_played] at @s if entity @e[tag=is_alive,sort=nearest,limit=1,distance=0..1] unless entity @e[tag=is_alive,sort=nearest,limit=1,distance=0..2,gamemode=creative] unless entity @e[tag=is_alive,sort=nearest,limit=1,distance=0..1,gamemode=spectator] run damage @e[tag=is_alive,sort=nearest,limit=1,distance=0..2] 0.1 mob_projectile by @s from @s
