...
execute as @e[tag=gothic_ghost,tag=ghost_type_poltergeist,sort=random,limit=1] if entity @s[scores={random100=..5}] at @s facing entity @e[tag=is_alive,sort=nearest,limit=1,distance=1..30] feet run summon egg ^ ^ ^1 {Motion:[-1.0,0.0,0.0],Item:{id:"minecraft:stone",Count:1b},Tags:['gothic_ghost_egg_thrown']}
