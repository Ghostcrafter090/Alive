...
try:
    __version__ = version("modules.pymodrinth")
...
class ModrinthClient:
    ...
    def __init__(
        ...
        user_agent: str = f"modules.pymodrinth/{__version__}",
        debug: bool = False,
    ...
    def _request(self, method: str, endpoint: str, api_version: int = 2, **kwargs) -> Any:
        ...
        session: Optional[requests.Session] = getattr(thread, "_modules.pymodrinth_session", None)
        if session is None:
            session = self._make_thread_session()
            setattr(thread, "_modules.pymodrinth_session", session)
