...
execute as @e[type=marker,tag=gstools_worker,scores={currentlyExecutingBossProgression=1..1}] run scoreboard players operation @s averageTpsBossProgressionWorkerMultTen /= @s 2
execute as @e[type=marker,tag=gstools_worker,scores={currentlyExecutingBossProgression=1..1}] run scoreboard players operation @s averageTpsBossProgressionWorkerMultTen += @s 40
