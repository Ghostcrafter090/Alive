# Update the date & time
    ...
    execute store success score #TimeLib.HourChanged TimeLib run data modify storage timelib_:zprivate CommandBlock.Time.Hour set string storage timelib_:zprivate CommandBlock.Output 10 12
    execute store success score #TimeLib.MinuteChanged TimeLib run data modify storage timelib_:zprivate CommandBlock.Time.Minute set string storage timelib_:zprivate CommandBlock.Output 13 15
    execute store success score #TimeLib.SecondChanged TimeLib run data modify storage timelib_:zprivate CommandBlock.Time.Second set string storage timelib_:zprivate CommandBlock.Output 16 18
    execute if score #TimeLib.HourChanged TimeLib matches 1 run function timelib_:zprivate/update_time/get_daytime/hour with storage timelib_:zprivate CommandBlock.Time
    execute if score #TimeLib.MinuteChanged TimeLib matches 1 run function timelib_:zprivate/update_time/get_daytime/minute with storage timelib_:zprivate CommandBlock.Time
    execute if score #TimeLib.SecondChanged TimeLib matches 1 run function timelib_:zprivate/update_time/get_daytime/second with storage timelib_:zprivate CommandBlock.Time
    ...
    execute if score #TimeLib.CommandBlock.Daytime TimeLib < #TimeLib.LatestUnixTimestamp.Daytime TimeLib run function timelib_:zprivate/update_time/get_unix_timestamp/adjust
...
# Run the '#timelib_:daytime_changed' event
# (Important): The number of passed seconds can be read in '#TimeLib.PassedSeconds TimeLib'.
function #timelib_:daytime_changed
scoreboard players reset #TimeLib.PassedSeconds

# Update the date storages, get the number of days that have passed and run the '#timelib_:date_changed' event if the date changed
# (Important): It changed if the previous tick's "Unix Timestamp without daytime" is different from the current.
execute unless score #TimeLib.CurrentDayUnixTime TimeLib = #TimeLib.PreviousDayUnixTime TimeLib run function timelib_:zprivate/update_time/set_date_storage
