...
# (Important): If the unix timestamp has changed, the time and daytime will need to refresh as well.
    ...
    # Convert the unix timestamp to date & time
        ...
        execute store result storage timelib_:zprivate Days.Input int 1 run scoreboard players operation #TimeLib TimeLib.Day %= #TimeLib.1461 TimeLib
        ...
        execute store result score #TimeLib TimeLib.Year run data get storage timelib_:zprivate Days.Out[0]
        execute store result score #TimeLib TimeLib.Month run data get storage timelib_:zprivate Days.Out[1]
        execute store result score #TimeLib TimeLib.Day run data get storage timelib_:zprivate Days.Out[2]
